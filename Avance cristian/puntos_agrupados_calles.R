...
# DEFINICIÓN DE RUTAS Y PARÁMETROS GLOBALES (Modifica aquí para cargar otro cluster)
...
# LÓGICA BIOMECÁNICA Y ESPACIAL DE TOLERANCIA
VELOCIDAD_MAX_MS <- 1.5  
UMBRAL_KMH <- VELOCIDAD_MAX_MS * 3.6 
MARGEN_CASA_M <- 10.0    # Metros máximos de ingreso a manzana permitido
...
cat(">>> Limpiando datos y ejecutando Algoritmos de Predicción y Corrección...\n")
...
# 1. FILTRO CINEMÁTICO (BIOMECÁNICO)
...
# --- NUEVO ALGORITMO: RUTA PREDICTIVA IMAGINARIA (Antes de modificar coordenadas) ---
# Tomamos los datos crudos válidos y aplicamos una "Media Móvil Espacial" para predecir el camino real fluido.
df_mapa_puro <- df_analisis %>% filter(ESTADO == "OK" & !is.na(Time))

generar_ruta_imaginaria <- function(df) {
  if(nrow(df) < 3) return(df)
  res <- df
  # Suavizado matemático: Combina el punto anterior, el actual (peso doble) y el siguiente.
...
# 2. FILTRO ESPACIAL: EXTRACCIÓN PERPENDICULAR PURA A LA CALLE
df_mapa <- df_mapa_puro # Usamos la copia para trabajar el mapa ajustado
...
# Base de Puntos Crudos para comparación
...
ui <- bootstrapPage(
  ...
  tabsetPanel(
    ...
    tabPanel("Auditoria: Puntos Correctos",
             div(style="padding: 30px; background-color: #f9f9f9; height: 95vh; overflow-y: auto;", h3("Registro de Puntos Validos (Filtrados)"), p("Puntos que superaron el Filtro Biomecanico y Espacial."), dataTableOutput("tabla_correctos"))
    ),
    tabPanel("Auditoria: Ruido y Errores",
             div(style="padding: 30px; background-color: #ffebee; height: 95vh; overflow-y: auto;", h3("Registro de Puntos Eliminados"), p("Detalle del ruido GPS descartado."), dataTableOutput("tabla_errores"))
    )
  )
)
...
server <- function(input, output, session) {
  ...
  output$mapa_interactivo <- renderLeaflet({
    ...
    if (!is.null(pol_sf)) m <- m %>% addPolygons(data = pol_sf, color = "#2c3e50", weight = 2, fillColor = "#bdc3c7", fillOpacity = 0.25, group = "Manzanas")
    
    # Se añaden la línea cruda con ruido y la NUEVA LÍNEA IMAGINARIA PREDICTIVA
    m <- m %>% 
    ...
    m <- m %>% addControl(html = leyenda_html, position = "bottomright") %>%
      addLayersControl(
        # SE AGREGA LA CAPA AL MENÚ INTERACTIVO
        overlayGroups = c("Manzanas", "Ruta Trazada", "Ruta Imaginaria (Predictiva)", "Puntos y Agrupaciones", "Puntos Corregidos Espacialmente", "Puntos Crudos (Con Ruido)"),
  ...
  observe({
    ...
    df_unicos <- df_cluster %>% filter(Puntos_Agrupados == 1) %>% 
      ...
      mutate(popup = paste0("<div style='text-align:center;'><b>Punto: ", IDs_Unidos, "</b><hr>Hora: ", format(Time_Min, "%H:%M:%S"), "<br>Situacion Geografica:<br><b>", Ubicacion, "</b><br>Velocidad: ", round(Vel_Ant_MS, 1), " m/s<br>Dist. al sgte: ", ifelse(is.na(Dist_Next), "Fin de ruta", paste0(round(Dist_Next, 1), " m")), "</div>"))
  ...
  output$tabla_correctos <- renderDataTable({
    df_mapa %>%
      mutate(Time = format(Time, "%H:%M:%S"), velocidad_ms = round(velocidad_ms, 2)) %>%
      select(ID_Original, Time, Longitude, Latitude, velocidad_ms, Ubicacion) %>%
      rename(`ID Punto` = ID_Original, `Hora` = Time, `Longitud` = Longitude, `Latitud` = Latitude, `Velocidad (m/s)` = velocidad_ms, `Situacion Geografica` = Ubicacion)
  ...
  output$tabla_errores <- renderDataTable({
    df_analisis %>%
      filter(ESTADO != "OK") %>%
      mutate(Time = format(Time, "%H:%M:%S"), dist_m = round(dist_m, 2), velocidad_ms = round(velocidad_ms, 2)) %>%
