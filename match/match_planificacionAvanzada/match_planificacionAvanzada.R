...
#Leer el archivos
  data_1<-read.csv("BD_Planificacion_avanzada.xlsx - PLANIFICACION_AVANZADA.csv", stringsAsFactors = FALSE)
  data_2<-read.csv("BD_Planificacion_avanzada2D.csv", stringsAsFactors = FALSE)
...
#Exportar en un archivo CSV el resultado del MATCH
  ...
  setwd('/home/gianfranco/Documentos/github/Participation/match/match_planificacionAvanzada/resultados')
  write.csv(data_problem, "datos_problema_match_planificacionAvanzada.csv")
  
##################################################
#
# MATCH PLANIFICACION AVANZADA: FASE 2
#
##################################################
  #Ruta donde se encuentran los archivos para la FASE 2
  setwd('/home/gianfranco/Documentos/github/Participation/match/match_planificacionAvanzada/correcciones')
  
  #Leer el archivos
    #Los datos corregidos deben tener un campo "ERRORES_DOCUMENTADOS" donde se explica el error que se encontro
      f2_match<-read.csv("datos_problema_match_planificacionAvanzada_CORREGIDO.csv")
    
  #ELIMINAR DATOS: 
    # - Datos que fueron tachados en las hojas pero fueron ingresados
    # - Datos que no se encuentra sus datos físicos
      f2_match <- f2_match[!grepl("ELIMINAR", f2_match$ERRORES_DOCUMENTADOS, fixed = TRUE),]# 199 registros eliminados
  ...
  #Almacenamos los datos que hicieron perfectamente el MATCH desde la FASE 1 hasta la FASE 2
    planiAvan_perfect <- f2_match$perfect
    result_match <- rbind(result_match, planiAvan_perfect)
  ...
  # y tambien los errores que se pudieron cometer al momento de corregir los registros
    planiAvan_problem <- f2_match$problem
  ...
  #------------------------------------------------------------------------------------------------------------------------------------
    #Ubicarmos solo a los unicode DUPLICADOS
      aux <- planiAvan_problem[which(duplicated(planiAvan_problem$UNICODE)),1]
      f2_duplicados<-planiAvan_problem[planiAvan_problem$UNICODE %in% aux,]
  ...
  #------------------------------------------------------------------------------------------------------------------------------------    
    #Aumentar registros correctos a la variable general, para tener el resultado final de el match, datos totalmente correctos.
      result_match_planiAvan <- rbind(result_match, planiAvan_problem)
    
    #Aplicando la funcion DataUnique que se encargara de que no haya registros exactamente iguales en el resultado
      result_match_planiAvan <- DataUnique(result_match_planiAvan)# Ningun registro eliminado
      
  #Exportar en un archivo CSV el resultado del MATCH
  #Ruta donde se almacenan los resultados
    setwd('/home/gianfranco/Documentos/github/Participation/match/match_planificacionAvanzada/resultados')
    write.csv(result_match_planiAvan, "result_match_planificacionAvanzada.csv")
